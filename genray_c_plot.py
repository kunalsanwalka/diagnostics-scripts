#%% Intialize data and print run details
# genray_c_plot.py'
...
filenm='C:/Users/kunal/OneDrive - UW-Madison/WHAM/Data/Genray/210505_newdens.nc'
#-----------------------------------------
...
# Specify for plots:
rhomax=2.0 #0. #3.0  # In the code, rhomax is defined as max of rho 
...
Zmin=0. #-190. #[cm] Z Limits for plots. Set to 0 if you want it done automatically
Zmax=0. #+190. #[cm] Z Limits for plots. Set to 0 if you want it done automatically          
            
fnt  = 22 #20 #18 #10     # font size for axis numbers (see 'param=' below) 
...
isp_wc=1 #1  #  Species number for which you want to plot omega/omega_c res.layers
...
i_cps=1 #Added [2018-10], for new feature "cross_polarization_scat"
...
if iplot_cql==1:
    ...
    print('========================================')
    print("The global attributes: ",list(dat_cql.dimensions.keys()))        
    print("File contains variables: ",list(dat_cql.variables.keys()))
    print('========================================')
    ...
    #--------------------------------------------------------------------------
    fig1=plt.figure(figsize=(15,15)) # RF power deposited to general species. 
    ...
    # plt.hold(True) #deprecated in python3
    ...
    ax = plt.subplot(1, 1, 1)
    # # plt.hold(True) #deprecated in python3 #deprecated in python3
    ...
    #--------------------------------------------------------------------------
    fig4=plt.figure(figsize=(15,15)) # Eplus accumulated over Z-grid. 
    ax = plt.subplot(1, 1, 1)
    # plt.hold(True) #deprecated in python3
    txt1= "$[kW]=$"+r"$%1.3f$" %(np.sum(prf_accum_allrays)) 
...
#================= genray.nc PLOTS ======================================
fig0=plt.figure(figsize=(15,15))  # B(r=0,z) vs Z coord.
ix0= int(nxeqd/2) #530
x00= eqdsk_x[ix0]
...
#-----------------------------------------------------------------------------
fig0=plt.figure(figsize=(15,15))  # B vs X coord. at y=0 z=0 plane
...
fig0=plt.figure(figsize=(15,15))  # omega_ce/omega profiles vs x coord.
f=freqcy  #[0]
...
n_scan=rscan_size-1
plt.plot(rscan[:],1./wcw[:],'b',linewidth=1.5,label='$ce$') # w/wce
# plt.text(Rmax*0.9, 1./wcw[n_scan],'$ce$', va='top',fontsize=fnt+4,color='b')
plt.plot(rscan[:],1./wpw[:],'r',linewidth=1.5,label='$pe$') # w/wpe
# plt.text(Rmax*0.75, 1./wpw[n_scan],'$pe$', va='top',fontsize=fnt+4,color='r')
plt.plot(rscan[:],1./wuw[:],'k',linewidth=1.5,label='$UH$') # w/wuh
...
if Nsp>1:  # ion species is present: plot omega/omega_LH
    plt.plot(rscan[:],1./wLH_w[:],'g',linewidth=1.5,label='$LH$') # w/wLH
...
fig0=plt.figure(figsize=(15,15))  # Characteristic frequencies vs R coord.
...
#--------------------------------------------------------------------------
    ...
    for i in range(0,inpar,1):
        fig0=plt.figure(figsize=(15,15))  # ddd(r,Nper,Npar) profiles vs x coord.
        # plt.hold(True) #deprecated in python3
...
#--------------------------------------------------------------------------
    for i in range(0,inpar,1):
        fig0=plt.figure(figsize=(15,15))  # Nperp2 profiles vs x coord.
        # plt.hold(True) #deprecated in python3
...
#%% Species density and temperature profiles
#--------------------------------------------------------------------------
fig0=plt.figure(figsize=(15,15))  # Te and ne profiles vs x or y coord.
plt.subplot(221)
# plt.hold(True) #deprecated in python3
plt.ylabel('$ T $   $ (keV) $')
plt.grid(True)
...
for i in range(0,Nsp,1):
    ...
    plt.plot(w_x_densprof_nc[:]*100,w_temp_vs_x_nc[i,:],color=col,linewidth=linw*(Nsp-i),label=isp_name[isp])
...
for i in range(0,Nsp,1):
    ...
    plt.plot(w_x_densprof_nc[:]*100,w_dens_vs_x_nc[i,:],color=col,linewidth=linw*(Nsp-i),label=isp_name[isp] )
...
for i in range(0,Nsp,1):
    ...
    if remainder(i,4)==2: col='r'
    if remainder(i,4)==3: col='g'    
    plt.plot(w_y_densprof_nc[:]*100,w_temp_vs_y_nc[i,:],color=col,linewidth=linw*(Nsp-i),label=isp_name[isp] )
...
for i in range(0,Nsp,1):
    ...
    if remainder(i,4)==2: col='r'
    if remainder(i,4)==3: col='g'    
    plt.plot(w_y_densprof_nc[:]*100,w_dens_vs_y_nc[i,:],color=col,linewidth=linw*(Nsp-i),label=isp_name[isp])  
...
#%% Parameter profiles at z=0
#--------------------------------------------------------------------------
if Nsp>1:   # For e or ions, if present
    fig0=plt.figure(figsize=(15,15))  # Valfven, nperp_Alfven, (Kperp*rho) profiles vs x or y coord., at z=0
    plt.subplot(221)
    # plt.hold(True) #deprecated in python3
...
#--------------------------------------------------------------------------
if i_cps==1:
    fig3=plt.figure(figsize=(15,15))  # starting values over (R,Npar) plane
    # plt.hold(True) #deprecated in python3
    plt.grid(True)
...
#%% Rays and electron density (XY Plot)
#--------------------------------------------------------------------------
fig4=plt.figure(figsize=(18,15))  # RAYS in top-view (R-phi or X-Y)
#ax = plt.subplot(1, 1, 1)
#ax.set_aspect(1.0)
# plt.hold(True) #deprecated in python3
plt.grid(True)
...
#%% Rays and electron density (XZ Plot)
#--------------------------------------------------------------------------
fig0=plt.figure(figsize=(15,15)) # RAYS and Density profile in cross-sectional view X-Z
ax = plt.subplot(111)
ax.set_aspect(1.0)
...
#%% Rays and electron density (YZ Plot)
#--------------------------------------------------------------------------
fig0=plt.figure(figsize=(22,15)) # RAYS and Density profile in cross-sectional view Y-Z
ax = plt.subplot(121)
ax.set_aspect(1.0)
...
#%% Rays and poloidal flux (XZ Plane)
#--------------------------------------------------------------------------
fig3=plt.figure(figsize=(15,10))  # RAYS and Pol.Flux in cross-section view X-Z
ax = plt.subplot(1, 1, 1)
ax.set_aspect(1.0)
plt.axis([Zmin,Zmax,Rmin,Rmax])
# plt.hold(True) #deprecated in python3
...
CS=plt.contour(Zr,-R,PSI[:],levels,linewidth=linw,cmap=cm.jet)
CB=plt.colorbar(orientation='vertical', shrink=1.0, format='%.1e') 

#Plot the field strength contours
R = dat['eqdsk_r'][:]
Z = dat['eqdsk_z'][:]
psi = dat['eqdsk_psi'][:]
dr = R[1]-R[0]
dz = Z[1]-Z[0]
Bz = np.gradient(psi,dr,axis=1) / R
Br = np.gradient(psi,dz,axis=0) / R
Bmag=np.sqrt(Bz**2+Br**2)

Rmesh,Zmesh = np.meshgrid(R, Z)
levels = np.linspace(1,25,25)    

CS=plt.contour(Zmesh,Rmesh,Bmag,levels=levels,cmap='plasma')
CS=plt.contour(Zmesh,-Rmesh,Bmag,levels=levels,cmap='plasma')

# if ios=='windows': 
#     CB.lines.set_linewidth(10)
CB.set_label("$(Tesla*m^2)$", size=fnt+4)
for i in range(0,Nrays,1):  # i goes from 0 to Nrays-1
    if remainder(i,6)==0: col='b'
    if remainder(i,6)==1: col='g'
    if remainder(i,6)==2: col='r'
...
plt.contourf(Zmesh,Rmesh,Bmag,levels=levels,cmap='plasma')
plt.contourf(Zmesh,-Rmesh,Bmag,levels=levels,cmap='plasma')
plt.colorbar()
plt.show()

#%% Rays and omega/omega_c (XZ Plane)
#-------------------------------------------------------------------------------
fig3=plt.figure(figsize=(15,15))  # RAYS and omega/omega_c in cross-section view X-Z
...
#%% B equilibrium field along ray path
#--------------------------------------------------------------------------
fig7=plt.figure(figsize=(15,15))  # B-equilibrium-field along RAYS vs distance(t)
plt.subplot(311) #-------------------------
# plt.hold(True) #deprecated in python3
plt.grid(True)
plt.ylabel('$B_Z$  $(G)$')
...
#%% E wave field along ray path
#--------------------------------------------------------------------------
fig8=plt.figure(figsize=(15,15))  # E-wave-field along RAYS vs distance(t)
plt.subplot(231) #-------------------------
# plt.hold(True) #deprecated in python3
plt.grid(True)
...
#%% E+- wave field along ray path
fig8=plt.figure(figsize=(15,15))  # E+ and E- wave-field along RAYS vs distance(t)
plt.subplot(611) #-------------------------
# plt.hold(True) #deprecated in python3
plt.grid(True)
plt.ylabel('$|E_{+}/E|$')
...
#stop   
#%% Miscellaneous parameters along ray path
fig8=plt.figure(figsize=(15,15))  # Z,R, Kperp*rho_Larm, V//res, Ki, along RAYS vs distance(t)
ax=plt.subplot(611) #-------------------------   
# plt.hold(True) #deprecated in python3
plt.grid(True)
plt.ylabel('$Z$'+' $(cm)$')
...
#%% n_par along ray path
fig8=plt.figure(figsize=(15,15))  #  N// along RAYS vs distance(t)
ax=plt.subplot(312) #-------------------------   
# plt.hold(True) #deprecated in python3
plt.grid(True)
...
#%% n_perp along ray path
if Nsp>1:
    isp=isp_wc-1 # ion (isp_wc is the genray-c index; isp is the python index)
    fig8=plt.figure(figsize=(15,15))  #  (omega/Kper)/Vti = (1/nper)/(Vti/c)      along RAYS
    ax=plt.subplot(312) #-------------------------   
    # plt.hold(True) #deprecated in python3
    plt.grid(True)
    ...
    show()   
#stop
#%% Velocities along ray path
#--------------------------------------------------------------------------
fig81=plt.figure(figsize=(15,15))  # Vgroup/c along RAYS vs distance(t)
plt.subplot(231) #-------------------------
# plt.hold(True) #deprecated in python3
plt.grid(True)
...
#stop
#%% Frequencies along ray path
#--------------------------------------------------------------------------
fig5=plt.figure(figsize=(15,15))  # wce/w and (wpe/w)^2 along RAYS vs distance
f=freqcy
...
#stop
#%% [DO NOT USE] Refractive indicies along ray path
#--------------------------------------------------------------------------
fig5=plt.figure(figsize=(15,15))  # Refractive indices along RAYS vs R(t)
ax=plt.subplot(231) #-------------------------
# plt.hold(True) #deprecated in python3
plt.grid(True)
...
#%% Refractive indicies along ray path
#--------------------------------------------------------------------------
fig6=plt.figure(figsize=(15,15))  # Refractive indices along RAYS vs distance(t)
plt.subplot(231) #-------------------------
# plt.hold(True) #deprecated in python3
plt.grid(True)
#plt.title('$N_R$ $vs$ $R$') # N_R ws R
...
#%% Power profiles as a function of rho
#--------------------------------------------------------------------------
fig7=plt.figure(figsize=(15,15))   # Power profiles
...
print('===========================================================')
#--------------------------------------------------------------------------
#fig1=plt.figure(figsize=(15,15))   # Current profiles
#plt.subplot(221) #-------------------------
...
#%% Electron power profile (RZ Plane)
#--------------------------------------------------------------------------
fig10=plt.figure(figsize=(15,15))   # Power profiles over R,Z grid
...
#%% Ion power profile (RZ Plane)
fig10=plt.figure(figsize=(15,15))   # Power profiles Pi over R,Z grid
ax=plt.subplot(111) #-------------------------
ax.set_aspect(1.0)
plt.axis([Zmin,Zmax,-Rmax,Rmax])
...
#%% Collision power profile (RZ Plane)
fig10=plt.figure(figsize=(15,15))   # Power profiles Pcl over R,Z grid
ax=plt.subplot(111) #-------------------------
ax.set_aspect(1.0)
plt.axis([Zmin,Zmax,-Rmax,Rmax,])
