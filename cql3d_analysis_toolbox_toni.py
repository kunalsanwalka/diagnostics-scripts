...
#Pleiades setup (installed on Dave in /home/wham)
...
# Plot Directory
# =============================================================================
plotDest='/home/sanwalka/plots/cql3d/'
...
def source_power_dens(filename,makeplot=False,saveplot=False):
    ...
    try:
        sorpw_nbi=np.array(ds['sorpw_nbi'][:])*1e5 #Convert to W/m^3
    except:
        sorpw_nbi=np.zeros((len(rya),ngen))
    
    #RF power density
    try:
        sorpw_rf=np.array(ds['sorpw_rf'][:])*1e5 #Convert to W/m^3
    except:
...
def fast_ion_confinement_time(filename,makeplot=False,saveplot=False,efastd=6):
    """
    This function calcualtes the fast ion confinement time as a function of the
    radius.
    ...
    rya : np.array
        Normalized radius of the flux tube
    tau_i : np.array
        Fast ion confinement time. Dimensions - (ngen,rya)
    """
    ...
    if makeplot==True:
        ...
        for i in range(ngen):
            ...
            ax.plot(rya,tau_i[i],label=labels[i])
...
def average_energy(filename,makeplot=False,saveplot=False):
    ...
    energyLastT : np.array
        Average energy of the ion species. Dimensions - (ngen,rya)
    """
    ...
    if makeplot==True:
        ...
        for i in range(ngen):
            ...
            ax.plot(rya,energyLastT[i],label=speciesLabels[i])
        ...
        if saveplot==True:
            ...
            #Remove the .nc part
            ncName=ncName[0:-3]
            savename=ncName+'_average_particle_energy.png'
            
            plt.savefig(plotDest+savename,bbox_inches='tight')
    
    return rya,energyLastT

def nbi_birth_points(makeplot=False,saveplot=False):
    """
    ...
    #Open the file
    fileObj=open('/home/sanwalka/cql3d/WHAM_VNS/freya_points.txt','r')
    ...
    for line in csv.reader(fileObj,delimiter=' '):
        ...
        #Calculate vr
        vr=np.sqrt(float(lineData[6])**2+float(lineData[7])**2)
        vrArr.append(vr)
    ...
    if makeplot==True:
        ...
        #Initialize the plot
        fig,axs=plt.subplots(3,1,figsize=(21,3*9))
    ...
    return xArr,yArr,zArr,rArr,vxArr,vyArr,vzArr,vrArr

def nbi_bounce_field(filename,makeplot=False,saveplot=False):
    ...
    xArr,yArr,zArr,rArr,vxArr,vyArr,vzArr,vrArr=nbi_birth_points()

    #WHAM VNS configuration
    wham=WHAM_VNS()

    #Pleiades grid limits
    rMax=np.max(rArr)
    zMax=np.max(zArr)
    zMin=np.min(zArr)
    ...
    #Create the mesh
    mesh=RectMesh(rmin=0,rmax=rMax,zmin=zMin,zmax=zMax,nr=nr,nz=nz)

    R=mesh.R
    Z=mesh.Z

    #Do the greens function calculation
    wham.mesh=mesh
...
filename='/home/sanwalka/cql3d/WHAM_VNS/210712_genray_61cm.nc'

#Get the netCDF4 data
ds=nc.Dataset(filename)

#Get the distribution function data
# distData,vPar,vPerp=dist_func(filename,makeplot=True,saveplot=True,fluxsurfplot=3,species=0)
...
#Get the integrated power density
# rya,sourceRate,integratedRF,totalIntegrated=integrated_power_density(filename,makeplot=True,saveplot=False)

#Get the fast ion confinement time
# rya,tau_i=fast_ion_confinement_time(filename,makeplot=True,saveplot=False)

#Get the average energy
# rya,energyLastT=average_energy(filename,makeplot=True,saveplot=True)

#Get the NBI bith points
# xArr,yArr,zArr,rArr,vxArr,vyArr,vzArr,vrArr=nbi_birth_points(makeplot=True)

#Get the NBI bounce field values
nbi_bounce_field(filename)

#Plot all distribution functions
# plot_dist_funcs(filename,saveplot=True,species=0)
